...
const Note = (props) => {
    ...
    const PostNote = notesData.map((post) => {
        return (
          <ItemsNote post={post} setActiveItem={setActiveItem} activeItem={activeItem} />
        );
    });
...
const ItemsNote = ({
    ...
    return (
      <Card className={classes.root} key={id} onClick={() => handleColor(classes.activeColor)} elevation={3}>
        <CardContent
          className="notes-wrapper"
          onClick={() => handleToggleClick(notesData.id)}>
          <CardContent className={color}>
            <Typography variant="h5">{title}</Typography>
            <Typography variant='subtitle2'>{description.substring(0, 20)}...</Typography>
            <div>{date}</div>
          </CardContent>
        </CardContent>
